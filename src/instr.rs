...
use crate::{
    ...
    runtime::{Num, Ref, Store, Val},
    types::{self, Index},
    validation::Context,
    vm::{Frame, Stack, StackEntry},
};
...
impl Instr {
    ...
    pub fn exec(&self, store: &mut Store, frame: &mut Frame, stack: &mut impl Stack) {
        match *self {
            // Numeric
            ...
            Instr::GlobalSet(global_idx) => {
                ...
                match stack.pop().unwrap() {
                    StackEntry::Value(val) => {
                        ...
                        store.globals[glob_addr].value = val;
                    }
                    _ => unreachable!(),
                }
            }
            Instr::GlobalGet(global_idx) => {
                // TODO validate index
                let glob_addr = frame.module.globals[global_idx];
                stack.push(StackEntry::Value(store.globals[glob_addr].value));
            }
            Instr::GlobalSet(global_idx) => {
                // TODO validate top of stack is value
                match stack.pop().unwrap() {
